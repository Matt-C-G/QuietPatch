...
def build_evidence_pack(input_file: str, html_file: str, output_zip: str, includes: List[str]):
    ...
    with zipfile.ZipFile(output_zip, 'w', zipfile.ZIP_DEFLATED) as z:
        ...
        # Add metadata
        metadata = {
            "generated": pathlib.Path(html_file).stat().st_mtime if Path(html_file).exists() else None,
            "includes": includes,
            "quietpatch_version": "0.2.1"
        }
        z.writestr("metadata.json", json.dumps(metadata, indent=2))

def _generate_csv_export(input_file: str) -> Optional[str]:
...
def _generate_sbom() -> Optional[str]:
...
def _generate_signatures(input_file: str, html_file: str) -> Optional[str]:
