...
sha_map={}
with open(root/"SHA256SUMS","r",encoding="utf-8") as f:
    for line in f:
        h, name = line.strip().split() if ' ' in line else (None,None)
        if h and name: sha_map[name]=h
def guess_platform(fname:str):
    f=fname.lower()
    if f.endswith(".pyz"): return ["macos","linux","windows"]
    if f.endswith(".cmd"): return ["windows"]
    if fname=="quietpatch": return ["macos","linux"]
    return []
artifacts=[]
for p in files:
    if p.name in ("SHA256SUMS","SHA256SUMS.minisig","minisign.pub"): continue
    artifacts.append({
        "file": p.name,
        "size": p.stat().st_size,
