from src.core.rollback import run_canary, snapshot_system_state
...
def test_snapshot_and_canary(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        rollback, "_get_snapshot_file", lambda: tmp_path / ".quietpatch" / "snapshot.json"
    )

    apps = [{"app": "Example", "version": "1.0"}]
    snapshot_system_state(apps)
