...
def compute_changes(outdir: Path) -> str | None:
    ...
    recipients = [
        r.strip() for r in os.environ.get("QP_AGE_RECIPIENTS", "").split(",") if r.strip()
    ]
    include_keychain = os.environ.get("QP_INCLUDE_KEYCHAIN_WRAP", "0") == "1"
    enc_path = outdir / "changes.json.enc"
    encrypt_file(
        changes_path, enc_path, age_recipients=recipients, include_keychain_wrap=include_keychain
    )
    return str(enc_path)
