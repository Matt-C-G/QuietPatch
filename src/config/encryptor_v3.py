...
def _age_unwrap(armored: bytes, identities: Iterable[str]) -> bytes:
    ...
    if not ids: raise DecryptError("No age identities provided")
...
def encrypt_bytes(plain: bytes,
                  age_recipients: Optional[Iterable[str]] = None,
                  include_keychain_wrap: bool = False) -> Dict[str, Any]:
    ...
    if include_keychain_wrap:
        if not _HAS_V2:
            ...
            try:
                ...
                wraps.append({"type":"keychain","alg":"AES-256-GCM","iv":iv2.hex(),"ct":ct2.hex()})
...
def decrypt_to_bytes_v3(env: Dict[str, Any],
                        *, age_identities: Optional[Iterable[str]]=None,
                        allow_keychain: bool=True) -> bytes:
    if env.get("v") != 3: raise DecryptError("Unsupported envelope")
    ...
    if ids:
        for w in env.get("wraps", []):
            if w.get("type") == "age-ssh":
                try:
                    dek = _age_unwrap(w["armored"].encode(), ids); break
                except Exception: pass
    if dek is None and allow_keychain and _HAS_V2:
        for w in env.get("wraps", []):
            if w.get("type") == "keychain":
                try:
                    kek = _get_backend().get_or_create_kek()  # type: ignore
                    dek = AESGCM(kek).decrypt(bytes.fromhex(w["iv"]), bytes.fromhex(w["ct"]), None); break
                except Exception: pass
    if dek is None: raise DecryptError("No usable wrap; provide --age-identity or enable keychain")
    return AESGCM(dek).decrypt(bytes.fromhex(env["iv"]), bytes.fromhex(env["ct"]), None)

def encrypt_file(plain_path: Path, enc_path: Path,
                 *, age_recipients: Optional[Iterable[str]]=None,
                 include_keychain_wrap: bool=False) -> None:
    env = encrypt_bytes(plain_path.read_bytes(),
...
def decrypt_file(enc_path: Path,
    ...
    if not t: raise DecryptError(f"Empty encrypted file: {enc_path}")
    try:
        env = json.loads(t)
    except json.JSONDecodeError:
        if _HAS_V2:
            try: return _v2_decrypt_to_bytes(json.loads(t))
            except Exception: pass
        raise DecryptError("Unknown/legacy envelope; regenerate with v3")
    if env.get("v") == 3: return decrypt_to_bytes_v3(env, age_identities=age_identities, allow_keychain=allow_keychain)
    if _HAS_V2: return _v2_decrypt_to_bytes(env)
