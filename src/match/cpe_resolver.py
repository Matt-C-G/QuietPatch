...
class CPEResolver:
    ...
    def _make_request(self, url: str, params: dict = None, headers: dict = None) -> dict | None:
    ...
    def build_candidate_cpes(self, app_name: str, version: str) -> list[str]:
        ...
        if version:
            ...
            for i in range(len(version_parts)):
                ...
                patterns.append(
                    f"cpe:2.3:a:{normalized_name}:{normalized_name}:{partial_version}:*:*:*:*:*:*:*:*"
                )

        return list(set(patterns))  # Remove duplicates

    def resolve_best_cpe(self, app_name: str, version: str) -> str | None:
    ...
    def search_nvd_cpe(self, app_name: str, version: str) -> str | None:
        """Search NVD for CPE matches (fallback method)"""
        try:
            # Search NVD CPE API
            search_url = "https://services.nvd.nist.gov/rest/json/cpes/2.0"
            params = {"keywordSearch": f"{app_name} {version}", "resultsPerPage": 5}
