...
t=[x for x in m["targets"] if x["os"]=="macos"][0]; print(os.path.join(os.path.dirname(sys.argv[1]), t["artifact"]))
PY
"$manifest")
...
t=[x for x in m["targets"] if x["os"]=="macos"][0]; print(t["sha256"]["artifact"])
PY
"$manifest")
